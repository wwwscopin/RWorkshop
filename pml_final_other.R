...
test<-rbind(one, test)
test$new_window[1]<-factor(c("yes"))
...
## Tree method! ###################################################
###################################################################
w<-tree(classe~., data=training)
summary(w)
plot(w);text(w)
pred<-as.data.frame(predict(w,testing))
pred$classe<-names(pred)[apply(pred,1, which.max)]

table(pred$classe, testing$classe)

err_rate<-1-sum(diag(table(pred$classe, testing$classe)))/dim(pred)[1]
err_rate

pred1<-as.data.frame(predict(w,test))
pred1$classe<-names(pred1)[apply(pred1,1, which.max)]
pred1$classe
###################################################################
